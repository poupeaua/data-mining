...
We notice that we get only 494 principal components instead of approximately 5000 which correspond to the number of genes. This can be explained because the PCA make linear combinaison to turn $5000$ components to $494$.
...
We predict only the coordinates where the ligns are equals to 1 in the set.
...
```
We can clearly see that, even when we change naxes, we have the predictedCoord that make sense compared to the theoryPosition.
...
Here we can see that by using the $4$ first PCA, we have a $2000$ kilometers difference. We expect that as we increase the number of PCA used, the model is better and predict more precisely the theorical coordinates
...
As we can see, the final graph shows that as we increase the number of PCs used for the regression at the beginning the difference of distance decrease. This is quiet expected because we have more information to describe the model. However, when the number of PCs used is too large it increases the difference : this is caused by overfitting. There is the limit where the model explains the errors that we don't want the model to explain. If we compare to the result obtained in the answer 4.b, we can see in abscissa $25$ that we have a difference of distance equal to $1400$. This result is really close to what we have found before $1100$.

According to this graph, we should use the first $80$ principals components to get the best prediction because this is approximately the minimum of the curve. This is the model we would keep.
